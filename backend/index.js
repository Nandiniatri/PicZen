...
app.post("/remove-subject", upload.single("image"), (req, res) => {
  const inputPath = req.file.path;
  const outputPath = `output/${Date.now()}.png`;

  exec(
    `python backend/remove_subject.py ${inputPath} ${outputPath}`,
    (error) => {
      if (error) {
        console.error("Python error:", error);
        return res.status(500).json({ error: "Subject removal failed" });
      }
...
app.get("/api/insert", (req, res) => {
  const data = JSON.parse(fs.readFileSync("./data/insert.json", "utf-8"));
  res.json(data);
});
